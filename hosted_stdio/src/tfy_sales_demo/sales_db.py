...
查询规则沿用原 Demo：值参数使用 SQLite 参数绑定，动态分组字段必须来自
白名单。18条虚构数据作为包资源读取，每次连接在内存中建立只读查询基线，
因此安装后不依赖源码目录、当前工作目录或外部数据库文件。
"""
...
class RequestValidationError(ValueError):
    """表示调用参数超出本 Demo 的明确查询范围。"""


@lru_cache(maxsize=1)
def _load_sales_rows() -> tuple[tuple[str, str, str, str, int], ...]:
    """只读取包内固定数据；缓存避免每次工具调用重复解析 JSON。"""

    resource = files("tfy_sales_demo").joinpath("data", "sales_records.json")
    with resource.open("r", encoding="utf-8") as stream:
        raw_rows = json.load(stream)
    rows = tuple(tuple(row) for row in raw_rows)
    if len(rows) != 18:
        raise RuntimeError("模拟数据资源不完整：预期18条记录。")
...
def get_data_description() -> dict[str, Any]:
    ...
    return {
        ...
        "amount_definition": "销售额为模拟含税销售额；内部按人民币分存储，接口同时返回元字符串。",
...
def safe_call(function: Any, **kwargs: Any) -> dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            "message": "查询执行失败，请查看托管日志或联系演示管理员。",
